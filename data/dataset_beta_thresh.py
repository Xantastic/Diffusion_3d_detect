...
class MVTec3DTrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        aug_img = (np.transpose(aug_img, (2, 0, 1))).astype(np.float32)/255.0
        ...
        if self.get_features:
            ...
            # 使用 pixel_shuffle 进行上采样，pixel_unshuffle就是通道变多，尺寸变小
            upscale_factor = 8
            features_m = nn.functional.pixel_shuffle(torch.from_numpy(features_m), upscale_factor)
            features_aug_m = nn.functional.pixel_shuffle(torch.from_numpy(features_aug_m), upscale_factor)
            # depth_t = nn.functional.pixel_unshuffle(torch.from_numpy(depth), upscale_factor)
            # depth_aug_t = nn.functional.pixel_unshuffle(torch.from_numpy(aug_depth), upscale_factor)
            # depth_t = depth_t.numpy()
            # depth_aug_t = depth_aug_t.numpy()
            # upscale_factor = 8
            # depth_m = nn.functional.pixel_unshuffle(depth, upscale_factor)
            # depth_aug_m = nn.functional.pixel_unshuffle(aug_depth, upscale_factor)
            # upscale_factor = 16
            # depth_b = nn.functional.pixel_unshuffle(depth, upscale_factor)
            # depth_aug_b = nn.functional.pixel_unshuffle(aug_depth, upscale_factor)

            # msk_t = downsampling(msk.squeeze(), (args["img_size"][0]/4, args["img_size"][0]/4))
            # msk_t = msk_t.reshape(1, args["img_size"][0]/4, args["img_size"][0]/4)


        sample = {'image': img_rgb,'depth': depth, 'sub_fg_depth': sub_fg_depth, 'is_anomaly': is_anomaly, 'aug_img': aug_img,'aug_depth':aug_depth,
                  'features_t': features_m, 'features_aug_t': features_aug_m,
                  # 'depth_t': depth_t, 'depth_aug_t': depth_aug_t,
...
class MVTec3DTestDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_gt = np.expand_dims(img_gt, axis=0).astype(np.float32)/255.0
        img_rgb = (np.transpose(img_rgb, (2, 0, 1))).astype(np.float32)/255.0
        depth = np.transpose(depth, (2, 0, 1)).astype(np.float32)
        ...
        if self.get_features: #[7,12,26] 1/4 1/8 1/16
            features_m = self.features_m[index]
            # features_m = self.features_m[index]
            # features_b = self.features_b[index]
